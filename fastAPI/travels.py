...
async def create_travels(travel: TravelsModel, Authentization: AuthJWT = Depends()):
    try:
        ...
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Token is invalid")
    exist_usere = session.query(Users).filter(Users.username == Authentization.get_jwt_subject()).first()
    if not exist_usere:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="The user does not exist")
    if exist_usere.is_staff:
                    ...
                    if exist_address:
                        ...
                        if exist_author:
                            ...
                            new_travel = Travels(
                                ...
                                places_id=new_places.id,
                                comments_id=new_comments.id,
                                discount=travel.discounts
                            )
